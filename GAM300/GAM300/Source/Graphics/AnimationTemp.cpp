...
// constructor
AnimationMesh::AnimationMesh(std::vector<AnimationVertex> vertices, std::vector<unsigned int> indices, std::vector<AnimationTexture> textures)
{
...
AnimationMesh AnimationModel::processMesh(aiMesh* mesh, const aiScene* scene)
{
    ...
    std::vector<AnimationTexture> diffuseMaps = loadMaterialTextures(material, aiTextureType_DIFFUSE, "texture_diffuse");
...
std::vector<AnimationTexture> AnimationModel::loadMaterialTextures(aiMaterial* mat, aiTextureType type, std::string typeName)
{
    std::vector<AnimationTexture> textures;
